...
function walkthourghPrefab(nodelist, entryindex, nodedata, parentname, parentoffsetX, parentoffsetY) {
  var width = 0;
  var height = 0;
  var offsetX = 0;
  var offsetY = 0;
  var scaleX_adj = 1;
  var scaleY_adj = 1;
  var nodename = nodelist[entryindex]._name;
  var namepath = (entryindex == 1) ? nodename : parentname + '/' + nodename;
  ...
  if (nodelist[entryindex]._components && nodelist[entryindex]._components.length > 0) {//檢查組件決定節點類別
    if (nodelist[entryindex]._components.find((component) => nodelist[component.__id__].__type__ == "cc.Button")) {
      ...
      if (componentIndex >= 0) {
        ...
        if (labelIndex >= 0 && nodelist[labelIndex]._isSystemFontUsed == false) {
      ...
      if (nodelist[spriteIndex]._type == 1 || nodelist[spriteIndex]._sizeMode == 0) {
    ...
    else if (nodelist[entryindex]._components.find((component) => nodelist[component.__id__].__type__ == "sp.skeleton")) {
      // if (entryindex != 1) {
      nodename = "sp_" + nodename;
      namepath = parentname + '/' + nodename;
      // }
    }
    // console.log('end');
  }
  var active = nodelist[entryindex]._active == true ? "True" : "False";
  var currentnode = nodedata.ele("AbstractNodeData", { Name: nodename, ActionTag: actionTag, Tag: 167 + entryindex, IconVisible: active, VisibleForFrame: active, ctype: ctype });
  ...
  if (nodelist[entryindex]._children.length > 0) {
    ...
    for (var i = 0; i < nodelist[entryindex]._children.length; i++) {
      if (nodelist[nodelist[entryindex]._children[i].__id__].__type__ && nodelist[nodelist[entryindex]._children[i].__id__].__type__ != "cc.PrivateNode") {//Creator內部使用的節點，不須轉換
        walkthourghPrefab(nodelist, nodelist[entryindex]._children[i].__id__, childrendata, namepath, offsetX, offsetY);
      }
    }
  }
  ...
  var scaleElement = currentnode.ele("Scale", { ScaleX: scale[0].toFixed(4).toString(), ScaleY: scale[1].toFixed(4).toString() });
  ...
  currentnode.att("Alpha", nodelist[entryindex]._color.a.toFixed(0).toString());
  ...
  switch (ctype) {
    ...
    case "SpriteObjectData":
      ...
      if (ctype == "SpriteObjectData") {
        var dst = nodelist[spriteIndex]._dstBlendFactor;
        var src = dst == 1 ? 770 : 1;
        currentnode.ele("BlendFunc", { Src: src.toFixed(0).toString(), Dst: dst.toFixed(0).toString() });
      }
    ...
    case "TextObjectData":
      anchorElement.att("ScaleX", nodelist[entryindex]._anchorPoint.x.toFixed(4).toString());
      anchorElement.att("ScaleY", nodelist[entryindex]._anchorPoint.y.toFixed(4).toString());
      var componentIndex1 = nodelist[entryindex]._components.findIndex((component) => nodelist[component.__id__].__type__ == "cc.RichText");
      var componentIndex2 = nodelist[entryindex]._components.findIndex((component) => nodelist[component.__id__].__type__ == "cc.Label");
      if (componentIndex1 >= 0) {
        var labelIndex = nodelist[entryindex]._components[componentIndex1].__id__;
        if (labelIndex >= 0) {
          currentnode.att("LabelText", nodelist[labelIndex]._N$string);
      ...
      else if (componentIndex2 >= 0) {
        var labelIndex = nodelist[entryindex]._components[componentIndex2].__id__;
        if (labelIndex >= 0) {
          currentnode.att("LabelText", nodelist[labelIndex]._N$string);
          currentnode.att("FontSize", nodelist[labelIndex]._fontSize.toFixed(0).toString());
          if (nodelist[labelIndex]._N$file != null) {
            var fontuuid = nodelist[labelIndex]._N$file.__uuid__;
            if (fileTable[fontuuid]) {
              var filepath = path.join("ArtTemp", "Fnt", fileTable[fontuuid].New);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (path.parse(filePath).ext == ".zip") {
        try {
          ...
          var csdHeader = path.join(cocosheader, "ArtTemp", "Csd");
          var imgHeader = path.join(cocosheader, "ArtTemp", "Img");
          var fntHeader = path.join(cocosheader, "ArtTemp", "Fnt");
          var particleHeader = path.join(cocosheader, "ArtTemp", "Particle");
          var spineHeader = path.join(cocosheader, "ArtTemp", "Spine");
          // if (fs.existsSync(tempheader)) {
          //   fs.removeSync(tempheader);
          // }
          fs.mkdirSync(desheader);
          fs.mkdirSync(cocosheader);
          // fs.mkdirSync(tempheader);
          fs.mkdirSync(path.join(cocosheader, "ArtTemp"));
          fs.mkdirSync(path.join(cocosheader, "ArtTemp", "Img"));
          fs.mkdirSync(path.join(cocosheader, "ArtTemp", "Csd"));
          fs.mkdirSync(path.join(cocosheader, "ArtTemp", "Particle"));
          fs.mkdirSync(path.join(cocosheader, "ArtTemp", "Fnt"));
          (async () => {
            ...
            var zipdata = await zip.loadAsync(zipfile, {
              decodeFileName: function (bytes) {
                return iconv.decode(bytes, 'Big5')  // 按中文编码
              }
            });
              ...
              if (ext == ".meta") {
                ...
                if (await zipdata.files[realpath]) {
                  ...
                  if (path.parse(filepath).ext == ".prefab") {
                    ...
                    prefabTable[uuid] = { Name: path.parse(filepath).name, Data: JSON.parse(prefabtext) };
                  }
                    ...
                    // var rawuuid = metadata.uuid;
                    fileTable[spriteuuid] = { Old: path.parse(filepath).base, New: newname, Path: path.join(imgHeader, newname) };
                    fileTable[uuid] = { Old: path.parse(filepath).base, New: newname, Path: path.join(imgHeader, newname) };
                    ...
                    fs.writeFileSync(path.join(imgHeader, newname), content);
                    imgFolder.ele("Image", { Name: newname });
                  }
                    ...
                    fileTable[uuid] = { Old: path.parse(filepath).base, New: newname, Path: path.join(fntHeader, newname) };
                    moveTable[imguuid] = { Name: imgname, desHeader: fntHeader };
                    fs.writeFileSync(path.join(fntHeader, newname), fnttext);
                    fntFolder.ele("Fnt", { Name: newname });
                  }
                    ...
                    // var fntuuid = metadata.uuid;
                    fileTable[uuid] = { Old: path.parse(filepath).base, New: newname, Path: path.join(fntHeader, newname) };
                    fs.writeFileSync(path.join(fntHeader, newname), content);
                    fntFolder.ele("Fnt", { Name: newname });
                  }
                    ...
                    // var imguuid = metadata.textureUuid;
                    fileTable[particleuuid] = { Old: path.parse(filepath).base, New: newname, Path: path.join(particleHeader, newname) };
                    // moveTable[imguuid] = particleHeader;
                    fs.writeFileSync(path.join(particleHeader, newname), particletext);
                    plistFolder.ele("PlistParticleFile", { Name: newname });
                  }
            ...
            for (var key in prefabTable) {
              ...
              csdname = getFolderUniqueName(csdHeader, csdname + ".csd");
              csdname = path.parse(csdname).name;
              // console.log(csdname);
              ...
              if (prefabData.Data[1]._children.length > 0) {
                ...
                for (var i = 0; i < prefabData.Data[1]._children.length; i++) {
                  ...
                  walkthourghPrefab(prefabData.Data, prefabData.Data[1]._children[i].__id__, childrendata, "Node", 0, 0);
                }
              }
                                ...
                                if (arraylength > 0) {
                                  ...
                                  switch (prop) {
                                    ...
                                    case "scale":
                                      ...
                                      if (startframe > 0) {
                                        var startX = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][0].value.x, 1);
                                        startX = (startX == 0) ? 0.01 : startX;
                                        var startY = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][0].value.y, 1);
                                        startY = (startY == 0) ? 0.01 : startY;
                                      ...
                                      for (var p = 0; p < arraylength; p++) {
                                        var frame = Math.floor(animTable[animuuid].curveData.paths[nodepath].props[prop][p].frame * 60);
                                        var x = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][p].value.x, 1);
                                        x = (x == 0) ? 0.01 : x;
                                        var y = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][p].value.y, 1);
                                        y = (y == 0) ? 0.01 : y;
                                      ...
                                      if (animlength > timelength) {
                                        var endX = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][arraylength - 1].value.x, 1);
                                        endX = (endX == 0) ? 0.01 : endX;
                                        var endY = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][arraylength - 1].value.y, 1);
                                        endY = (endY == 0) ? 0.01 : endY;
                                    ...
                                    case "scaleX":
                                      ...
                                      if (startframe > 0) {
                                        var valueX = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][0].value, 1);
                                        valueX = (valueX == 0) ? 0.01 : valueX;
                                        var valueY = 1;
                                      ...
                                      for (var p = 0; p < arraylength; p++) {
                                        ...
                                        var valueX = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][p].value, 1);
                                        valueX = (valueX == 0) ? 0.01 : valueX;
                                        var valueY = 1;
                                      ...
                                      if (animlength > timelength) {
                                        ...
                                        var valueX = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][arraylength - 1].value, 1);
                                        valueX = (valueX == 0) ? 0.01 : valueX;
                                        var valueY = 1;
                                    ...
                                    case "scaleY":
                                      ...
                                      if (startframe > 0) {
                                        var valueY = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][0].value, 1);
                                        valueY = (valueY == 0) ? 0.01 : valueY;
                                        var valueX = 1;
                                      ...
                                      for (var p = 0; p < arraylength; p++) {
                                        var frame = Math.floor(animTable[animuuid].curveData.paths[nodepath].props[prop][p].frame * 60) + offsetLength;
                                        var valueY = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][p].value, 1);
                                        valueY = (valueY == 0) ? 0.01 : valueY;
                                        var valueX = 1;
                                      ...
                                      if (animlength > timelength) {
                                        var frame = offsetLength + animlength;
                                        var valueY = defaultNumber(animTable[animuuid].curveData.paths[nodepath].props[prop][arraylength - 1].value, 1);
                                        valueY = (valueY == 0) ? 0.01 : valueY;
                                        var valueX = 1;
                                ...
                                if (animTable[animuuid].curveData.paths[nodepath].comps["cc.Sprite"]) {
                                  if (animTable[animuuid].curveData.paths[nodepath].comps["cc.Sprite"].spriteFrame) {
                                    ...
                                    if (arraylength > 0) {
                                      ...
                                      //若節點帶序列圖動畫，變更原本圖片節點為精靈節點
                                      var spritenode = nodeData.find(n => n.node.getAttribute("ActionTag") == actiontag, true, true);
                                      if (spritenode) {
                                        spritenode.att("ctype", "SpriteObjectData");
                                      }
                                    }
                                  }
                                }
