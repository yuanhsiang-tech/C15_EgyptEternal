<template>
  <li v-show="item.show">
    <div class="title" @click.stop="toggle($event)" @mouseover.stop="triggerHover" @mouseleave.stop="triggerLeave">
      <div id="line" class="fill"></div>
      <input type="checkbox" style="margin-top:4px;margin-right:5px;min-width:16px;min-height:16px" v-model="isSelected" @change="checkSelect" @click.stop="" :disabled="isAssociated" />
      <i v-if="item.isDir" :class="[isOpen ? 'fa fa-folder-open open' : 'fa fa-folder close', 'file-icon']"></i>
      <i v-else class="fa fa-file-text green file-icon" aria-hidden="true" @click="toggle"></i>
...
export default {
  ...
  methods: {
    ...
    triggerHover: function() {
      ...
      this.$bus.$emit("reset-right");
      this.$bus.$emit("check-right", this.item.path);
    ...
    checkSelect: function() {
      if (this.item.isDir && this.item.children.length > 0) {
    ...
    checkDown: function(check) {
      if (this.item.show) {
        ...
        if (this.item.isDir && this.item.children.length > 0) {
          ...
          this.$bus.$emit("recheck-file", this.item.path, check);
          this.$bus.$emit("select-right", this.item.path, check);
        }
      }
    },
    checkUp: function() {
